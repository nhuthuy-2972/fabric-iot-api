...
module.exports.getDataDevice = async (channelName, username, org_name, deviceID) => {
    ...
    try {
        ...
        result = await contract.evaluateTransaction(process.env.GETDATAFUNC, deviceID);
...
module.exports.pushDataDevice = async (channelName, username, org_name, deviceID, data) => {
    ...
    try {
        ...
        if (!identity) {
            ...
            // identity = await wallet.get(username);
            console.log('Run the registerUser.js application before retrying');
            return new Error(`An identity for the user ${username} does not exist in the wallet`)
        }
        // console.log(identity.credentials.certificate);
        const gateway = new Gateway();
        await gateway.connect(ccp, {
